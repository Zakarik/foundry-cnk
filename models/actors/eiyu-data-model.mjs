...
export class EiyuDataModel extends foundry.abstract.TypeDataModel {
	static defineSchema() {
		const {EmbeddedDataField, SchemaField, StringField, NumberField, BooleanField, ArrayField, HTMLField} = foundry.data.fields;
        const itm = new ActorsModels('eiyu');
        let data = itm.integration();
        data.capacites = new SchemaField({
            max:new NumberField({initial:0}),
            value:new NumberField({initial:0}),
        });
        data.mythos = new NumberField({initial:0});
        ...
        data.physique = new HTMLField({initial:""});
        data.origine = new StringField({initial:""});
        data.religion = new StringField({initial:""});
    ...
    prepareDerivedData() {
        ...
        Object.defineProperty(this.capacites, 'value', {
            value: getVoie.ptscapacites,
        });

        Object.defineProperty(this.capacites, 'max', {
            value: profil === undefined ? 0 : 2+(this.niveau.actuel*2)+profil?.system?.capacite ?? 0,
        });
    }
